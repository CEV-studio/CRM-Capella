...
export function Ligne({ p, afficherCommercial, selectionnable = false, coche = false, onToggle, etapes = PROSPECT_STAGES }: {
  ...
  return <>
    <tr className={cn("border-b border-navy-100 hover:bg-navy-50", coche && "bg-star-50", notesOuvertes && "bg-navy-50")}>
      ...
      <td className={cn(CELL,"min-w-56 max-w-72")}><Link href={`/prospection/${p.id}`} prefetch={false} className="block truncate font-semibold text-navy-800 hover:text-star-600" title={p.raison_sociale ?? undefined}>{p.raison_sociale || nomComplet(p.nom,p.prenom)}</Link><div className="tabular text-[11px] text-grey-brand">{p.ref}{p.siren ? ` · ${p.siren}` : ""}</div></td>
      ...
      <td className={cn(CELL,"tabular whitespace-nowrap text-navy-700")}>{(() => { const numero=p.tel_mobile||p.tel_fixe; return numero ? <a href={`tel:${numero}`} className="hover:text-star-600">{numero}{!p.tel_mobile ? <span className="ml-1 text-[11px] text-grey-brand">(fixe)</span> : null}</a> : "—"; })()}</td>
      <td className={CELL}><MenuEtape id={p.id} etapeEnBase={p.stage} etapes={etapes} couleur={(e)=>stageColor(e,"prospect")} action={actionEtape} resultat={etatEtape} libelle={`Étape de ${p.raison_sociale ?? "ce prospect"}`} /></td>
