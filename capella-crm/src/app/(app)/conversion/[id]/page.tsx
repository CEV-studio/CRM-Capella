...
export default async function FicheAffairePage({
  ...
  // RLS s'applique : une affaire qui n'est pas la sienne renvoie « introuvable ».
  ...
  const [
    ...
    { data: piecesData },
      ...
      a.prospect_id
        ? supabase
            .from("prospects")
            .select("id, ref, raison_sociale")
            .eq("id", a.prospect_id)
            .is("deleted_at", null)
            .maybeSingle()
        : Promise.resolve({ data: null }),
      // Pièces de l'affaire + celles héritées du prospect d'origine.
      supabase
        .from("pieces_jointes")
        .select("*")
        .or(
          a.prospect_id
            ? `affaire_id.eq.${a.id},prospect_id.eq.${a.prospect_id}`
            : `affaire_id.eq.${a.id}`,
        )
        .order("created_at"),
    ]);

  const prospect = origine as Pick<Prospect, "id" | "ref" | "raison_sociale"> | null;

  const toutesPieces = (piecesData ?? []) as PieceJointe[];
  const piecesAffaire = toutesPieces.filter((pj) => pj.affaire_id === a.id);
  const piecesHeritees = toutesPieces.filter(
    (pj) => pj.affaire_id !== a.id && pj.prospect_id === a.prospect_id,
  );
    ...
    <main className="mx-auto w-full max-w-5xl px-6 py-8">
      ...
      <header className="mt-3 mb-6">
        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-2 text-sm text-grey-brand">
          <StageBadge label={a.stage} color={stageColor(a.stage, "affaire")} />
          <span className="tabular">{a.ref}</span>
          <span>· entrée le {fmtDate(a.date_entree)}</span>
          {prospect ? (
            <span>
              · issue du prospect{" "}
              <Link
      ...
      <div className="mt-6">
        <PiecesJointes
          scope="affaire"
          parentId={a.id}
          pieces={piecesAffaire}
          heritees={piecesHeritees}
        />
      </div>
    </main>
  );
