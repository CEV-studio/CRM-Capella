...
export default async function TableauDeBordPage({ searchParams }: { searchParams: Promise<Recherche> }) {
  ...
  const [{ data: affairesBrutes }, { data: prospectsBruts }, { data: profils }, apporteurs] = await Promise.all([
    supabase.from("affaires").select("commercial_id, apporteur_id, stage, date_signature, date_relance, commission").is("deleted_at", null),
  ...
  const prospects = (prospectsBruts ?? []) as Pick<Prospect, "stage" | "next_action_date">[];
