...
export default async function ClientsPage({ searchParams }: { searchParams: Promise<Recherche> }) {
  ...
  const resultat: any = await db
    .from("prospects")
    .select("id, ref, raison_sociale, nom, prenom, stage")
    .is("deleted_at", null)
    ...
    <main className="w-full px-6 py-8">
      <header className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-grey-brand">Dossiers à partir de Demande ACD, avant passage en cotation.</p>
        </div>
            ...
            <div className="divide-y divide-navy-100">
              {clients.map((c) => (
                <Link key={c.id} href={`/prospection/${c.id}`} className="flex items-center justify-between gap-4 px-4 py-3 hover:bg-navy-50">
                  ...
                  <span className="rounded-full px-2.5 py-1 text-xs font-semibold text-navy-800" style={{ backgroundColor: stageColor(c.stage, "prospect") }}>{c.stage}</span>
                </Link>
              ))}
            </div>
          )}
