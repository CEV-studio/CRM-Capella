...
export function ProspectNoteEditor({ prospectId, onNotesChange }: {
  prospectId: string;
  initialNotes?: string | null;
  compact?: boolean;
  onNotesChange?: (notes: ProspectNote[]) => void;
}) {
  ...
  function replaceNotes(next: ProspectNote[]) {
    setNotes(next);
    onNotesChange?.(next);
  }
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled) replaceNotes(data.notes ?? []);
      } catch (err) {
  ...
  async function ajouter() {
    ...
    try {
      const response = await fetch(`/api/prospects/${prospectId}/notes`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ body: text }) });
      ...
      replaceNotes([data.note, ...notes]);
      setBody("");
      requestAnimationFrame(() => { if (textareaRef.current) textareaRef.current.style.height = "42px"; textareaRef.current?.focus(); });
    } catch (err) { setError(err instanceof Error ? err.message : "Ajout de la note impossible."); }
  ...
  async function modifier(noteId: string) {
    ...
    setBusyNoteId(noteId); setError(null);
    try {
      const response = await fetch(`/api/prospects/${prospectId}/notes`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ noteId, body: text }) });
      const data = await response.json() as { note?: ProspectNote; error?: string };
      if (!response.ok || !data.note) throw new Error(data.error || "Modification impossible.");
      replaceNotes(notes.map((note) => note.id === noteId ? data.note! : note));
  ...
  async function supprimer(noteId: string) {
    ...
    setBusyNoteId(noteId); setError(null);
    try {
      const response = await fetch(`/api/prospects/${prospectId}/notes`, { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ noteId }) });
      const data = await response.json() as { ok?: boolean; error?: string };
      if (!response.ok || !data.ok) throw new Error(data.error || "Suppression impossible.");
      replaceNotes(notes.filter((note) => note.id !== noteId));
  ...
  return (
    <div>
      <div className="flex items-end gap-2 rounded-xl border border-navy-200 bg-white p-2 shadow-sm transition focus-within:border-sky-capella-300 focus-within:ring-2 focus-within:ring-sky-capella-100">
        <textarea ref={textareaRef} rows={1} value={body} onChange={(e) => { setBody(e.target.value); resize(); }} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); void ajouter(); } }} placeholder="Ajouter une note…" className="min-h-[42px] flex-1 resize-none overflow-y-auto border-0 bg-transparent px-2 py-2 text-sm leading-5 text-navy-900 outline-none placeholder:text-navy-300" />
        <button type="button" onClick={() => void ajouter()} disabled={saving || !body.trim()} className="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-lg bg-star-500 px-3 text-xs font-bold text-white shadow-sm transition hover:bg-star-600 disabled:bg-navy-100 disabled:text-navy-300 disabled:shadow-none"><Plus size={14}/>{saving ? "Ajout…" : "Ajouter"}</button>
      </div>
      <div className="mt-1.5 flex items-center justify-between gap-3 px-1 text-[10px] text-grey-brand"><span>Entrée pour ajouter · Maj+Entrée pour aller à la ligne</span>{notes.length ? <span className="font-semibold text-navy-500">{notes.length} note{notes.length > 1 ? "s" : ""}</span> : null}</div>
      {error ? <p className="mt-2 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700">{error}</p> : null}

      {loading ? <div className="mt-3 rounded-xl bg-navy-50 px-4 py-4 text-xs text-grey-brand">Chargement des notes…</div> : notes.length ? (
          ...
          {notes.map((note) => (
            <article key={note.id} className="relative flex gap-3">
              ...
              <div className="min-w-0 flex-1 rounded-xl border border-navy-100 bg-white p-3 shadow-sm transition hover:border-sky-capella-200">
                <div className="flex flex-wrap items-start justify-between gap-2"><div><div className="text-xs font-bold text-navy-900">{note.author_name}</div><div className="mt-0.5 flex flex-wrap items-center gap-2 text-[10px] text-grey-brand"><time dateTime={note.created_at}>{fmtNoteDate(note.created_at)}</time>{note.updated_at ? <span className="italic text-star-600">modifiée {fmtNoteDate(note.updated_at)}</span> : null}</div></div>{note.can_edit ? <div className="flex items-center gap-1"><button type="button" onClick={() => { setEditingId(note.id); setEditingBody(note.body); setError(null); }} className="rounded-md p-1.5 text-navy-400 hover:bg-sky-capella-50 hover:text-sky-capella-700" aria-label="Modifier la note"><Pencil size={13}/></button><button type="button" onClick={() => void supprimer(note.id)} disabled={busyNoteId === note.id} className="rounded-md p-1.5 text-navy-400 hover:bg-red-50 hover:text-red-700 disabled:opacity-40" aria-label="Supprimer la note"><Trash2 size={13}/></button></div> : null}</div>
