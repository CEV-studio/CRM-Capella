...
export function CalendarPanel({ prospectId, prospectEmail, prospectLabel, connected, accountEmail, events }: {
  ...
  return <>
    {connected ? <button type="button" onClick={() => { setOpen(true); setStatus(null); }} className="inline-flex h-9 w-full items-center justify-center gap-2 rounded-xl border border-star-200 bg-star-50 px-2 text-xs font-semibold text-star-800 shadow-sm hover:border-star-300 hover:bg-star-100"><AlarmClock size={14}/>Rappels & RDV{sortedEvents.length ? <span className="rounded-full bg-white px-1.5 py-0.5 text-[10px] shadow-sm">{sortedEvents.length}</span> : null}</button> : <a href={`/api/calendar/connect?returnTo=${encodeURIComponent(`/prospection/${prospectId}`)}`} className="inline-flex h-9 w-full items-center justify-center gap-2 rounded-xl border border-navy-200 bg-white px-2 text-xs font-semibold text-navy-700 shadow-sm hover:border-navy-300 hover:bg-sky-capella-50"><CalendarDays size={14}/>Connecter l’agenda</a>}
