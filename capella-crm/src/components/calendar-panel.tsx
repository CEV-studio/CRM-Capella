...
function eventClasses(kind: CalendarEvent["kind"]) {
  return kind === "rappel" ? "border-amber-200 bg-amber-50/80" : "border-sky-200 bg-sky-50/80";
}
...
export function CalendarPanel({ prospectId, prospectEmail, prospectLabel, connected, accountEmail, events }: {
  ...
  const modal = open ? (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-navy-900/55 p-4" role="dialog" aria-modal="true" aria-label="Rappels et rendez-vous" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
        ...
        <div className="grid gap-5 p-5 lg:grid-cols-[1fr_0.9fr]">
          <div>
            ...
            <div className="grid gap-3">
              ...
              <label className="text-xs font-semibold text-navy-700">{kind === "rdv" ? "Date & heure de la présentation" : "Date & heure du rappel"}<input type="datetime-local" value={startLocal} onChange={(e) => setStartLocal(e.target.value)} className="mt-1 h-10 w-full rounded-lg border border-navy-200 px-3 text-sm" /></label>
              {kind === "rdv" ? <label className="text-xs font-semibold text-navy-700">Durée<select value={durationMinutes} onChange={(e) => setDurationMinutes(Number(e.target.value))} className="mt-1 h-10 w-full rounded-lg border border-navy-200 bg-white px-3 text-sm"><option value={15}>15 min</option><option value={30}>30 min</option><option value={45}>45 min</option><option value={60}>1 h</option><option value={90}>1 h 30</option><option value={120}>2 h</option></select></label> : null}
              <label className="text-xs font-semibold text-navy-700">Alerte Google<select value={reminderMinutes} onChange={(e) => setReminderMinutes(Number(e.target.value))} className="mt-1 h-10 w-full rounded-lg border border-navy-200 bg-white px-3 text-sm"><option value={0}>Aucune</option><option value={10}>10 min avant</option><option value={30}>30 min avant</option><option value={60}>1 h avant</option><option value={1440}>1 jour avant</option></select></label>
              ...
              {kind === "rdv" && prospectEmail ? <label className="flex items-start gap-2 rounded-lg bg-sky-50 px-3 py-2 text-xs text-navy-700"><input type="checkbox" checked={inviteClient} onChange={(e) => setInviteClient(e.target.checked)} className="mt-0.5" /><span>Inviter le client à <strong>{prospectEmail}</strong>. L’invitation indiquera la date et l’heure de présentation.</span></label> : null}
              <button type="button" onClick={() => void creer()} disabled={saving || !startLocal} className={kind === "rappel" ? "h-10 rounded-lg bg-amber-500 px-4 text-sm font-semibold text-white hover:bg-amber-600 disabled:opacity-50" : "h-10 rounded-lg bg-sky-600 px-4 text-sm font-semibold text-white hover:bg-sky-700 disabled:opacity-50"}>{saving ? "Création…" : kind === "rappel" ? "Créer le rappel" : "Créer le RDV comparatif"}</button>
          ...
          <div className="border-t border-navy-100 pt-4 lg:border-l lg:border-t-0 lg:pl-5 lg:pt-0">
            ...
            {sortedEvents.length ? <div className="space-y-2">{sortedEvents.slice(0, 8).map((event) => <div key={event.id} className={`rounded-lg border p-3 ${eventClasses(event.kind)}`}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="truncate text-xs font-semibold text-navy-800">{event.kind === "rappel" ? "⏰ " : "📅 "}{event.title}</div><div className="mt-1 text-[11px] font-bold text-navy-700">{fmtEventDate(event.start_at)}</div>{event.location ? <div className="mt-1 truncate text-[11px] text-grey-brand">{event.location}</div> : null}</div><button type="button" onClick={() => void supprimer(event)} disabled={deletingId === event.id} className="shrink-0 text-xs text-grey-brand hover:text-red-700 disabled:opacity-40" title="Supprimer">×</button></div>{event.html_link ? <a href={event.html_link} target="_blank" rel="noreferrer" className="mt-2 inline-block text-[10px] font-semibold text-navy-700 underline underline-offset-2">Ouvrir dans Google ↗</a> : null}</div>)}</div> : <p className="rounded-lg bg-navy-50 px-3 py-6 text-center text-xs text-grey-brand">Aucun rappel ou RDV à venir.</p>}
          </div>
        </div>
      </div>
    </div>
  ...
  return <>
    {connected ? <button type="button" onClick={() => { setOpen(true); setStatus(null); }} className="inline-flex h-9 w-full items-center justify-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-2 text-xs font-semibold text-amber-800 hover:bg-amber-100"><span aria-hidden>⏰</span>Rappels & RDV{sortedEvents.length ? <span className="rounded-full bg-white px-1.5 py-0.5 text-[10px]">{sortedEvents.length}</span> : null}</button> : <a href={`/api/calendar/connect?returnTo=${encodeURIComponent(`/prospection/${prospectId}`)}`} className="inline-flex h-9 w-full items-center justify-center gap-2 rounded-lg border border-navy-200 bg-white px-2 text-xs font-semibold text-navy-700 hover:bg-navy-50"><span aria-hidden>📅</span>Connecter l’agenda</a>}
