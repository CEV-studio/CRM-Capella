 ...
 *
 * Ce fichier reflète les migrations SQL de `supabase/migrations/`.
 * Il pourra être régénéré automatiquement plus tard avec :
 *   npx supabase gen types typescript --project-id <ID> > src/lib/domain/database.types.ts
 */
...
export type Source = {
  id: string;
  name: string;
  kind: "call_center" | "apporteur" | "fichier" | "web" | "autre";
  is_active: boolean;
  created_at: string;
}
...
/** Définition d'un champ personnalisé (ex. « Marge souhaitée »). */
export type ChampPersonnalise = {
  id: string;
  /** Clé normalisée, stable, utilisée dans prospects.champs_perso. */
  cle: string;
  /** Libellé affiché (ce que Jeremy a tapé). */
  libelle: string;
  created_at: string;
}
...
export type Prospect = {
  ...
  pdl: string | null;
  pce: string | null;
  car_electricite: number | null;
  car_gaz: number | null;
  option_tarifaire: string | null;
  fournisseur_electricite: string | null;
  fournisseur_gaz: string | null;
  date_fin_contrat: string | null;
  /** Valeurs des champs personnalisés, rangées par clé (voir ChampPersonnalise). */
  ...
  siren_norm: string | null;
  pdl_norm: string | null;
  pce_norm: string | null;
...
export type Affaire = {
  ...
  type_energie: TypeEnergie | null;
  contrat: string | null;
  pdl_elec: string | null;
  pce_gaz: string | null;
  stage: string;
...
/** Champs modifiables par l'application (les colonnes calculées sont exclues). */
type Writable<T, Generated extends keyof T = never> = Omit<T, Generated>;

export type ProspectInsert = Partial<
  Writable<
    Prospect,
    "id" | "ref" | "siren_norm" | "pdl_norm" | "pce_norm" | "mobile_norm" | "created_at" | "updated_at"
  >
>;
export type ProspectUpdate = ProspectInsert;

export type AffaireInsert = Partial<
  Writable<Affaire, "id" | "ref" | "created_at" | "updated_at">
> & { raison_sociale: string; commercial_id: string };
export type AffaireUpdate = Partial<AffaireInsert>;

export type PieceJointeInsert = Partial<Writable<PieceJointe, "id" | "created_at">> & {
  type: "ACD" | "Facture";
  bucket_path: string;
  file_name: string;
};
